...
import {clearQueryCache, getQueryCache, setQueryCache} from '../redis/redis';
import {EQueryCache} from './enums/EQueryCache';
import DocumentArray = Types.DocumentArray;

export const getOwnPurchaseHistory = catchAsync(
    ...
    const groups: Types.DocumentArray<IGroupGameDocument> = await GroupGame.aggregate([
        {
            $project: {
                ...
                totalInvested: 1,
                totalMembers: {
                    $cond: {if: {$isArray: '$members'}, then: {$size: '$members'}, else: 'NA'},
                },
            },
        },
    ])
...
export const addMemberToGroup = catchAsync(async (req: Express.Request, res: Express.Response, next: NextFunction) => {
    ...
    await clearQueryCache(EQueryCache.getUser, req.user.toString());
    await clearQueryCache(EQueryCache.getGameGroups, req.user.toString());
...
export const removeMemberFromGroup = catchAsync(
    async (req: Express.Request, res: Express.Response, next: (err: any) => void) => {
        const relationData = matchedData(req);
        ...
        await clearQueryCache(EQueryCache.getGameGroups, relationData.userId.toString());
    },
);

export const getPurchaseHistory = catchAsync(
    async (req: Express.Request, res: Express.Response, next: (err: any) => void) => {
...
export const getGroupWinnersTop = catchAsync(
    async (req: Express.Request, res: Express.Response, next: NextFunction) => {
...
const getGroupsByUser = async (userId: string | ObjectID, res: Express.Response, next: NextFunction) => {
    let groups: IGroupGameDocument[] | undefined;
    const cacheObject = await getQueryCache(EQueryCache.getGameGroups, userId.toString());
    if (!!cacheObject) {
    ...
    if (!groups) {
        groups = await GroupGame.find({'members.userId': new ObjectID(userId)});
        await setQueryCache(EQueryCache.getGameGroups, userId.toString(), JSON.stringify(groups));
    }
    res.status(200).json(groups);
};
