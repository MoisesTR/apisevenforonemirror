    ...
    async (req: Express.Request, res: Express.Response, next: NextFunction) => {
        ...
        if (!user) {
            ...
            if (user.enabled === false) {
                ...
                return next(
                    new AppError('Usuario deshabilitado, contacte con el soporte de 7x1!.', 400, 'EPUSER'),
                );
            }
            if (decode.isExpired) {
                if (req.cookies[ECookies._RefreshToken]) {
                    try {
                        ...
                        if (decodedRefresh.isExpired) {
                            return next(
                                new AppError('Refresh token expired, please log again!', 401, 'TOKENEXPIRED'),
                            );
                        }
...
export const isInRole = (...roles: ObjectId[]) => (
    req: Express.Request,
