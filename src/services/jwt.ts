...
export const createAccessToken = async (
    user: IUserDocument,
    expiration: number = envVars.ACCESS_TOKEN_DURATION,
    unitOfTime: DurationInputArg2 = envVars.ACCESS_TOKEN_MEASURE,
) => {
    return createToken(
        {
            sub: user._id,
...
export const createRefreshToken = async (
    user: IUserDocument,
    expiration: number = envVars.REFRESH_TOKEN_DURATION,
    unitOfTime: DurationInputArg2 = envVars.REFRESH_TOKEN_MEASURE,
) => {
    return createToken(
        {
            sub: user._id,
...
export const ensureAuth = catchAsync(async (req: Express.Request, res: Express.Response, next: NextFunction) => {
    ...
    // Verifying the cache
    const cacheObject = await getQueryCache(EQueryCache.getUser, decode.sub);
    if (cacheObject) {
        logger.info('Using the cache for the user ' + decode.sub);
        user = new User(JSON.parse(cacheObject));
    } else {
        user = await User.findById(decode.sub);
        logger.info('Setting the user cache');
        await setQueryCache(EQueryCache.getUser, decode.sub, JSON.stringify(user));
    }
